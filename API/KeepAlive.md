...
opt with App PoL
  ...
  alt App restart, POL --x OB before incoming session & App not LBd yet
    Note over A:  restart
    Note over O:  OB not getting POL before interval
    Note over O:  invalidate LB
    Note right of O: incoming session ←
    Note over O: app not locally bound →
    Note over A,O:  local binding
    Note over A,O:  normal flow
  else App restart, incoming session before POL --x OB 
    Note over A: restart
    Note right of O: incoming session ←
    Note over O: SSE incoming session attempt
    Note over O: incoming session timer expires
    Note over O: DECLINE invite →
    Note over A,O:  local binding
    Note over A,O:  normal flow
  else App restart, incoming session before POL --x OB, LB before timer
    Note over A: restart
    Note right of O: incoming session ←
    Note over O: SSE incoming session attempt
    Note over A,O:  local binding & channel restoration
    Note over O: reissue SSE incoming session? new timer?
  end
end

opt without App PoL
  alt App restart, LB before timer
    Note over A: restart
    Note right of O: incoming session ←
    Note over O: SSE incoming session attempt
    Note over A,O:  local binding & channel restoration
    Note over O: reissue SSE incoming session? new timer?
  else App restart, LB before timer
   Note over A: restart
   Note right of O: incoming session attempt ←
   Note over O: SSE incoming session attempt
   Note over O: incoming session timer expires
